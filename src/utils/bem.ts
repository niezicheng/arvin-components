...
const createModifierClass = (modifier: TBemModifier, prefixClass?: string) => {
  ...
  if (isArray(modifier)) {
    // modifier 去重
    return [...new Set(modifier)].map((item) => ` ${prefixClass}--${item}`).join('');
  }
...
export const createCssScope = (block: string, element?: string | string[], identity = 'ar') => {
const elementPrefix = isArray(element)
? element.map((item) => (item ? `__${item}` : '')).join(' ')
: isString(element)
? `__${element}`
: '';
const prefixClass = `${identity}-${block.replace(identity, '')}${elementPrefix}`;

  return (modifier?: TBemModifier, modifierLater?: TBemModifier) => {
    const modifierClass = modifier ? createModifierClass(modifier, prefixClass) : '';
    console.log('%c👉  modifierClass: ', 'background:#41b883;padding:1px; border-radius: 0 3px 3px 0;color: #fff', modifierClass) // 👈
    const modifierLaterClass = modifierLater ? createModifierClass(modifierLater, prefixClass) : '';
